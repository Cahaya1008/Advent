...
window.openDay10 = function(){
  ...
  window.showModal(`<div style="padding:12px"><div style="display:flex;justify-content:space-between;align-items:center"><h3 style="color:var(--gold)">Dec 10 — Sudoku (9×9)</h3><button class="ghost" onclick="hideModal()">Exit</button></div>
    <p>Classic Sudoku. Use up to <strong>3 hints</strong>. Revealed hints persist across resets.</p>
    <div id="sudWrap" style="margin-top:12px"></div>
    <div style="margin-top:8px"><button class="primary" id="sudReset">Reset</button> <button class="primary" id="sudHint">Hint (<span id="hintLeft">${window._AC_state.sudokuHintsLeft}</span>)</button> <button class="ghost" id="sudExit">Exit</button></div>
  ...
  function checkComplete(){
    ...
    alert('Sudoku solved! Letter U unlocked.'); window.storeLetter(10);
    return true;
  }
  ...
  document.getElementById('sudHint').addEventListener('click', ()=>{
    ...
    for(let r=0;r<9;r++) for(let c=0;c<9;c++){
      ...
      if(window._AC_state.sudokuRevealed && window._AC_state.sudokuRevealed[key]) continue;
    ...
    alert('No empty cell available for hint.');
  });
