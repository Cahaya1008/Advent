...
window.openDay4 = function(){
    ...
    window.showModal(`
      <div style="padding:12px">
        <div style="display:flex;justify-content:space-between;align-items:center">
          <h3 style="color:var(--gold)">Dec 4 — Jigsaw (Found)</h3>
          <button class="ghost" onclick="hideModal()">Exit</button>
        </div>
        <p>Letter: <strong>V</strong></p>
      </div>`, {size:'lg'});
    return;
  }

  // Use your custom image
  const imageURL = 'img/day4.jpg';
  const tilesPerRow = 6;
  ...
  window.showModal(`
    <div style="padding:12px">
      <div style="display:flex;justify-content:space-between;align-items:center">
        <h3 style="color:var(--gold)">Dec 4 — Jigsaw</h3>
        <button class="ghost" onclick="hideModal()">Exit</button>
      </div>
      ...
      <div style="margin-top:10px">
        <button class="primary" id="shuffle4">Shuffle</button>
        <button class="primary" id="inspect4" disabled>Inspect</button>
      </div>
    </div>`, {size:'lg'});
  ...
  function shuffle(){ 
    ...
    render(); 
    document.getElementById('inspect4').disabled = true; 
  }

  function checkSolved(){ 
    for(let i=0;i<tiles.length;i++) if(tiles[i]!==i+1) return false; 
    document.getElementById('inspect4').disabled=false; 
    alert('Puzzle assembled! Click Inspect to reveal the back.'); 
    return true; 
  }
  ...
  document.getElementById('inspect4').addEventListener('click', ()=>{
    window.showModal(`
      <div style="padding:12px">
        <h3>Back of puzzle</h3>
        <p>The letter on the back is: <strong>V</strong></p>
        <div style="margin-top:8px">
          <button class="primary" onclick="window.storeLetter(4);hideModal()">Collect</button>
          <button class="ghost" onclick="hideModal()">Exit</button>
        </div>
      </div>`, {size:'sm'});
  });
