...
window.openDay7 = function(){
  if(window._AC_state.found[7]){
    window.showModal(`<div style="padding:12px"><div style="display:flex;justify-content:space-between;align-items:center"><h3 style="color:var(--gold)">Dec 7 — Color Sequence (Found)</h3><button class="ghost" onclick="hideModal()">Exit</button></div><p>Letter: <strong>F</strong></p></div>`, {size:'sm'});
    return;
  }
  window.showModal(`<div style="padding:12px"><div style="display:flex;justify-content:space-between;align-items:center"><h3 style="color:var(--gold)">Dec 7 — Color Sequence (Simon)</h3><button class="ghost" onclick="hideModal()">Exit</button></div>
    <p>Pass at least 5 rounds (3→7 length) to collect the letter. Bonus fast round is optional and intentionally hard.</p>
    <div style="display:flex;gap:12px;margin-top:12px" id="simRow">
    ...
    <div style="margin-top:12px"><button class="primary" id="start7">Start</button> <span id="s7msg" class="muted" style="margin-left:12px"></span></div>
  </div>`, {size:'sm'});
  ...
  document.getElementById('start7').addEventListener('click', async ()=>{
    ...
    // bonus optional fast round
    msg.textContent='Bonus (optional): fast round';
