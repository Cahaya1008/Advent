...
window.openDay11 = function(){
  if(window._AC_state.found[11]){ window.showModal(`<div style="padding:12px"><h3>Dec 11 — Found</h3><p>Letter: <strong>I</strong></p><div style="margin-top:10px"><button onclick="hideModal()">Close</button></div></div>`); return; }
  window.showModal(`<div style="padding:12px"><h3 style="color:var(--highlight)">Dec 11 — Minesweeper</h3>
    <p>25×25 with 100 bombs. First-click safety guaranteed. Left click reveals; right click toggles flag.</p>
  ...
  function render(){
    area.innerHTML=''; for(let r=0;r<rows;r++){ for(let c=0;c<cols;c++){ const btn=document.createElement('div'); btn.style.width='18px'; btn.style.height='18px'; btn.style.display='inline-flex'; btn.style.alignItems='center'; btn.style.justifyContent='center'; btn.style.border='1px solid #c6d6ee'; btn.style.background = revealed[r][c] ? '#fff' : '#e9f3ff'; btn.style.fontSize='11px'; btn.style.cursor='pointer';
          if(flagged[r][c] && !revealed[r][c]) btn.textContent='🚩';
          else if(revealed[r][c]){ if(grid[r][c]===-1) btn.textContent='💣'; else if(grid[r][c]===0) btn.textContent=''; else btn.textContent=grid[r][c]; }
  ...
  function revealCell(r,c){
    if(r<0||r>=rows||c<0||c>=cols) return; if(revealed[r][c]||flagged[r][c]) return; revealed[r][c]=true;
    if(grid[r][c]===0) for(let dr=-1;dr<=1;dr++) for(let dc=-1;dc<=1;dc++) if(!(dr===0&&dc===0)) revealCell(r+dr,c+dc);
  }
  function onReveal(r,c){
    if(!placed){ placeBombs(r,c); }
    if(grid[r][c]===-1){
      ...
      let won=true; for(let i=0;i<rows;i++) for(let j=0;j<cols;j++) if(grid[i][j]!==-1 && !revealed[i][j]) { won=false; break; }
      if(won){ alert('You cleared the minefield! Letter I collected.'); window.storeLetter(11); window.hideModal(); }
    }
  }
